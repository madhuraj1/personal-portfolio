...
export default function CelebrationChapter(props: Props) {
  return (
    <div className="relative -z-10 bg-stone-100 py-14 pt-44">
      <div className="relative mx-auto mb-24 flex max-w-[1480px] items-center justify-center overflow-hidden rounded-xl py-36">
        <div className="touch__carousel absolute inset-0 z-[-1] overflow-hidden xs:hidden md:block">
          <EmblaCarousel slides={[]} {...props} />
        </div>
        <div className="touch__carousel absolute inset-0 z-[-1] overflow-hidden xs:block md:hidden">
          <img src={props.portraitImageForBackground.url} alt="" />
        </div>
        <div className="mx-4 w-full rounded-2xl bg-white bg-opacity-40 p-12 py-24 text-center shadow-lg md:w-[calc(100vw-30rem)] md:rounded-[100px]">
          ...
          <div className="flex items-center justify-center gap-4">
            <button className="rounded-full bg-white px-8 py-3 text-lg text-black transition-colors hover:bg-slate-300">
              Get Quote
            </button>
            <button className="rounded-full bg-black px-8 py-3 text-lg text-white transition-colors hover:bg-gray-800">
              Contact Us
            </button>
          </div>
        </div>
